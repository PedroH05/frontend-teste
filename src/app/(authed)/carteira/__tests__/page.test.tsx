...
describe('CarteiraPage', () => {
  ...
  it('pagina a tabela de processos de 12 em 12, igual ao Histórico', async () => {
    const user = userEvent.setup();
    ...
    expect(screen.getByText('CLIENTE12')).toBeInTheDocument();
    expect(screen.queryByText('CLIENTE13')).not.toBeInTheDocument();
    expect(screen.getByText(/Página 1 de 2/)).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: 'Próxima ›' }));

    expect(screen.getByText('CLIENTE13')).toBeInTheDocument();
  ...
  it('volta pra página 1 quando um filtro muda o conjunto exibido', async () => {
    const user = userEvent.setup();
    const rows = Array.from({ length: 13 }, (_, i) => row({ capId: i + 1, cli: `CLIENTE${i + 1}` }));
    apiFetchMock.mockResolvedValueOnce({ rows });
    render(<CarteiraPage />);
